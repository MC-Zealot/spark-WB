...
object KolFansTagsScore_new {
  def main(args: Array[String]) {
    ...
    val sqlDF = spark.sql("select uid,fans_tags from uid_fans_tags where dt=20180102")
...
//        ((x(0),  fields(0)), 1)
    ...
    }.reduceByKey((x,y)=>(x._1+y._1,x._2+y._2)).map(x=>(x._1._1+"\t"+x._1._2+"\t"+x._2._1+"\t"+x._2._2)).saveAsTextFile("/user_ext/ads_fanstop/yizhou/spark/user_similarity/kol_ratings/0103")
...
//做一个过滤，粉丝数》50，标签保留人数的top50
    ...
    sc.textFile("/user_ext/ads_fanstop/yizhou/spark/user_similarity/kol_ratings/0103").filter(_.split("\t").length==4).filter(_.split("\t")(3).toInt>=50).repartition(100).
      map(x=>(x.split("\t")(0),x)).groupByKey().map{x=>
      val uid = x._1
      val tags = x._2
      tags.toSeq.sortWith(_.split("\t")(3).toInt > _.split("\t")(3).toInt).take(50)
    }.
      saveAsTextFile("/user_ext/ads_fanstop/yizhou/spark/user_similarity/kol_ratings/filtered/0104_2")
  }
}
